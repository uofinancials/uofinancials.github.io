...
test('a data file is fetched from the base path and parsed', async () => {
  ...
  await expect(testQueryClient().fetchQuery(manifestQuery)).resolves.toEqual(
    EMPTY_MANIFEST,
  )
...
test('a budget year is fetched by its two-digit file name', async () => {
  serve(404, {})
  await expect(
    testQueryClient().fetchQuery(budgetYearQuery(2027)),
  ).rejects.toThrow('Could not load /data/budget/FY27.json: HTTP 404')
})

test('a file that does not match its schema fails with its path', async () => {
  serve(200, { ...EMPTY_MANIFEST, rates: 'none' })
  await expect(testQueryClient().fetchQuery(manifestQuery)).rejects.toThrow(
