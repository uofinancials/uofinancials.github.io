...
import { DATA_DIR, FALL_SOURCE_DIR, listPdfs, MANIFEST_PATH } from './cache.ts'
...
async function readManifest(): Promise<Manifest> {
  if (!existsSync(MANIFEST_PATH)) return { fall: [] }
...
async function main(): Promise<void> {
  ...
  if (byCensus.size === 0 && problems.length === 0) {
    console.error(`No PDFs found. Download the Fall Census reports from
